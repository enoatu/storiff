...
## 仕組み
- prep がカレントの作業差分を読み、差分のかたまりからステップの区切りを機械で作る
- serve が Node単一ファイルの極小ローカルビューアを立て、URL をその場で返す
- fill が説明文を後ろから埋め、ビューアが3秒ごとに拾って描き直す
...
- ファイルをまたいでまとめない
- 題はかたまりの見出しから取った仮のもの、説明は空。説明は後から fill が埋める
...
## 読みながら説明文が埋まる
下書きの説明文は空なので、そこを `node storiff.js fill <dir>` が埋める。ステップ1つにつき `claude -p` を1つ立て、4本まで同時に走らせる。

肝は、全部書き終わるのを待たないこと。1つ書けたらその場で `steps.json` に戻す。ビューアは3秒ごとに差分を取りに行って変わったところだけ描き直すので、1コマ目を読んでいる間に後ろのコマが埋まっていく。手元の36ステップの差分では、1コマ目が読めるようになるまで24秒、36コマ全部が埋まりきるまで442秒だった。読み手が待つのは最初の24秒だけで、残りは読んでいる後ろで進む。

説明文が長いほど書き終わるまで待たされるので、1コマは300文字程度、3行から5行に抑えている。子プロセスに渡すのは担当するステップの題と変更IDの範囲だけで、材料の `changes.txt` と `context.txt` と `hints.txt` も担当の範囲だけ読めばよいと伝える。全部読ませると1コマあたりの時間が伸びる。

- まだ埋まっていないコマは、画面に「準備中」と出す。空欄のまま何も出さないと壊れて見えるため
- ステップの題は prep が付けた仮のままにして書き換えない。読んでいる途中で左の目次の並びがずれないようにするため
- すでに説明文があるステップには触らない。手で書き直した分は残る
- 先頭のステップから順に埋める。読み手は1コマ目から読むため
- 途中で止めても、そこまでに書けた分は残る。もう一度叩けば残りだけを埋める
- `claude` が入っていない環境では静かに飛ばし、説明文は空のままにする

`steps.json` に書き戻すのは親プロセスだけで、子プロセスは説明文を標準出力で返すだけにしてある。詳しくは [docs/story-schema.md](docs/story-schema.md) の「説明文の埋め戻し」を参照。
...
## 使い方
1. `/storiff` を実行する
2. すぐにURLが返るので、ブラウザで開いて1コマ目から読み始める。説明文は後ろで埋まっていくので、まだのコマには「準備中」と出る
3. ステップを読みながら気になる行にコメントする。各ステップの理解度クイズに答えると次へ進める
...
| ファイル | 役割 |
| --- | --- |
| `storiff.js` | prep(差分解析)と fill(説明文の埋め戻し)と serve(ビューア配信)を行うNode単一ファイル |
