...
説明文が長いほど書き終わるまで待たされるので、1ステップは300文字程度、3行から5行に抑えている。子プロセスに渡すのは担当するステップの題と変更IDの範囲だけで、材料の `changes.txt` と `context.txt` と `hints.txt` も担当の範囲だけ読めばよいと伝える。全部読ませると1ステップあたりの時間が伸びる。

- まだ埋まっていないステップは、画面に「準備中」と出す。空欄のまま何も出さないと壊れて見えるため。ただし「補足」と「修正N回目」のステップは、説明文が空でも準備中とは出さない
...
| owns に入っている割合が半分(50%)未満 | ng。ストーリーとして成立していないとみなす |

抜けはそのステップに出てこないだけでビューアは壊れないので、既定では ng にしない。ok の行は `ok: 全100件の変更IDのうち82件(82%)が step の owns に入っています` になり、割り当てがどこまで届いたかが数で分かる。

`--strict` を付けると抜けも ng になり、書き戻しもしない。ok の行は `ok: 全100件の変更IDがちょうど1回ずつ owns に入っています` になる。ストーリーをすべて合わせると指定した差分になることを確かめられるのは `--strict` だけなので、owns を書き換えた後は必ず付ける。
...
## 使い方
1. `/storiff` を実行する
2. すぐにURLが返るので、ブラウザで開いて最初のステップから読み始める。説明文は後ろで埋まっていくので、まだのステップには「準備中」と出る
