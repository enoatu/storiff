...
## 検算をゆるめる指定
`node storiff.js check <dir>` は既定で、全変更IDがちょうど1回ずつどれかのステップに入ることを求める。AI はこれが ok になるまで割り当てを直し続けるので、ストーリー作りの時間の多くがここに使われる。どれだけ効いているかを測りたいときのために、検算をゆるめる指定を2つ用意している。

| 指定 | 動き |
| --- | --- |
| `--no-strict` | どのステップにも入らなかった変更IDを ng にせず、参考として出すだけにする |
| `--backfill` | どのステップにも入らなかった変更IDを末尾の「補足」ステップに入れ、steps.json を書き戻す。`--no-strict` も一緒に効く |

重複した変更IDと不明なファイルは、どちらの指定でも今まで通り ng になる。重複はその変更行がどのステップのものか決まらずビューアが壊れるが、抜けはそのステップに出てこないだけで壊れないため。

どちらかを付けると、ok の行が `ok: 全 100 件の変更IDのうち 82 件(82%)が owns に入っています` になり、割り当てがどこまで届いたかが数で分かる。何も付けなければ検算も出力も今まで通り厳格なままで、`/storiff` の流れは変わらない。
