...
- 関数・クラス・型はどの深さで書かれていても拾う。変数と定数は行頭にあるものだけ拾う(JavaScript の `export const`、Python の代入、Go の `var` と `const`)。関数の中で作った作業用の変数は境界の手がかりにならないので落とす
...
- 行の頭に書かれた名前を1行につき1つしか拾わない。`const [left, right] = pair` のような分割代入や、Go の `a, b := f()` は左端しか拾えない
