...
- `node storiff.js check <dir> [--no-strict] [--backfill]` : `steps.json` を検算し、抜けID・重複ID・不明ファイルを表示する。ok なら `ok: 全 N 件の変更IDがちょうど1回ずつ owns に入っています` と出し、quiz を持つステップが1つでもあれば `(理解度クイズ M問)`、diagram を持つステップが1つでもあれば `(図 M枚)` を末尾に足す。quiz を持つステップは作りも確かめ、diagram を持つステップは図の書き方も確かめ、不備があれば ng にして理由を並べる。詳しくは「理解度クイズ」と「ステップの図」を参照。1stepの変更行が目安(80行)を超えると参考として一覧に出し、目安の2倍を超えてかつ複数ファイルにまたがると ng になり分割を求める。overview を持つときはその作りも確かめ、空や0件のところを参考として出す(ng にはしない)
  - `--no-strict` を付けると、抜けIDを ng にせず参考として出すだけにする。重複IDと不明ファイルは今まで通り ng。重複はその変更行がどのstepのものか決まらずビューアが壊れるが、抜けはそのstepに出てこないだけで壊れない
  - `--backfill` を付けると、抜けIDを末尾の `補足` stepに入れて steps.json を書き戻す。`補足` stepがすでにあれば作り直さずそこに足す。narration は空で、後で人か AI が書く。`--no-strict` も一緒に効く
  - どちらかを付けたときの ok は `ok: 全 N 件の変更IDのうち M 件(P%)が owns に入っています` になり、どれだけ覆えたかが分かる。付けなければ検算も出力も今まで通り
