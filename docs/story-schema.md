...
- `node storiff.js check <dir>` : `steps.json` を検算する。全変更IDがちょうど1回ずつどれかのステップの owns に入っているときだけ ok で、`ok: 全N件の変更IDがちょうど1回ずつ owns に入っています` と出す。quiz を持つステップが1つでもあれば `(理解度クイズ M問)`、diagram を持つステップが1つでもあれば `(図 M枚)` を末尾に足す
  ...
  - quiz を持つステップは作りを、diagram を持つステップは図の書き方を確かめ、不備があれば ng にして理由を並べる。詳しくは「理解度クイズ」と「ステップの図」を参照
...
ステップごとに1問だけ置く選択式の問い。読み飛ばしを防ぎ、他人に説明できるところまで理解が届いたかを本人が確かめるために使う。任意の項目なので、無いステップは素通りできる。

既定では作らない。`~/.storiff/config.json` に `{"quiz": true}` を書いたときだけ、prep が `理解度クイズ: 有効。各ステップに quiz を1問つける` の1行を出し、skill はそれを見てステップごとに quiz を作る。設定を読むのは prep だけなので、書き換えた後は同じ `<dir>` で prep を実行し直すまで skill に伝わらない。後から設定を消しても、すでに quiz を持つ steps.json はそのまま動き、check もその作りを確かめる。
...
- 「このステップは何をしましたか」「追加された関数の名前はどれか」「変更した行は何行か」のような、読んだり数えたりすれば答えが出る問いにしない
...
- quiz の answer も渡るので、開発者ツールを開けば正解は見える。自分の理解を確かめる道具であって、点を競う試験ではない
