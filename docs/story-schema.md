...
- 行コメントの change_id も新IDに写る。写せないコメントは change_id が null になり、body と replies は残る。ビューアは「修正で無くなった行へのコメント」としてステップの末尾に出す。どのステップにも属さないコメントも最後のステップに寄る。scope と resolved はどの範囲でもそのまま残る
...
- 追従では scope と resolved がそのまま写る。change_id を写すのは `line` だけで、写せなければ null になる。`step` と `story` は行を持たないので追従で消えない
