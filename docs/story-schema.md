...
- 区切りの下書き: かたまりから機械的に作ったステップの並び。題と説明は空で、後から AI が書き直す前提のもの
...
- title は仮のもの。かたまりの見出し(`@@` の後ろ)が取れていればそれを、無ければファイル名を入れる。長い見出しは40文字で切る。割ったステップの2つ目からは末尾に `の続き` が付く
- narration は空文字。AI が後から書く
- title と narration が空のまま配信すると、ステップの意図が何も書かれていない紙芝居になる。下書きは AI が書き直す前の土台であって、そのまま読ませるものではない
- かたまりは近い変更行がまとまっているだけなので、意図が同じでも離れていれば別のステップに散る。AI はこの下書きをそのまま採らず、意図に合わせてまとめ直してよい
...
narration が空であること自体が「まだ書けていない」印になるので、steps.json に別の印は持たない。
